...
**2026-09-08 — a seed-based map generator** (same "compiles clean, not Editor-verified" caveat): `MapGenerator.Generate` builds a fair, reproducible `LevelData` — one Throne Room with an attached Portal Room per player, a minimal starting domain each (3×3 Treasury, 4×4 Lair, 4 Imps), and a resource-wall scatter that's *equal between keepers by construction* (the whole per-player layout is a grid-aligned rotation of one local template — 2 players mirror, 3–4 take the map corners — and each resource vein is only committed where it's valid for every player's transform, so gold and mana yields match exactly). Because it emits a `LevelData`, it rides the existing loaded-level path unchanged. Reachable from a new Main Menu **"Skirmish (generated)"** button, the multiplayer lobby's **Procedural** map option (now a real N-keeper map — it used to build a single keeper), and a **"Generate Map"** button in the Level Designer's properties screen (seed + Randomize). Not generated yet: terrain features, the other four economy rooms, fauna, props.
...
- [ ] Extend the map generator — terrain features (Water/Lava/Chasm/Holy/Unholy), the other four economy rooms, fauna, treasure props; a seed field in the lobby + Skirmish (both use a random seed today)
