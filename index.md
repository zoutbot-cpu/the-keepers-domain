...
Phase 1's core loop is implemented and playable: **dig → claim → build → impling appears.** Beyond that, the prototype has grown a resource economy, three creature types, seven room types, and a debug/UI layer well past the original Phase 1 brief.
...
**Creatures** — Shared `Creature` base (HP, mana, stats, leveling 1–10, naming). Three creatures live:
...
**Rooms** — Lair, Treasury, Slime Hatchery, Bacon Beacon, Training Room, Library, Jail (placement + visuals only — see below). All sellable through one generic Sell tool; all merge cleanly when extended into a bigger rectangle.
...
- **Jail** — footprint, pit, fence, and staircase visuals are done; the actual capture/prisoner mechanic and its target creature (Maze Rattler) are **not implemented**.
...
- Additional creature races beyond Imp/Gremlin/Warlock
...
- [ ] Design and implement the Jail capture/prisoner mechanic + Maze Rattler creature
