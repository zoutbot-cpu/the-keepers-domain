...
- Hard 4x5 minimum footprint, checked in either orientation (at least 4 in one dimension and 5 in the other) — same "reject anything smaller than the minimum in either dimension" enforcement Jail/Slime Hatchery/Bacon Beacon use, just two different numbers instead of one square one. 20 gold per tile.
