...
- **Contested claiming**: territory grows into a *rival's* claimed floor as well as into unclaimed ground. `BuilderJobBoard.ScanForEnemyClaimCandidates` (an always-on 1 s sweep, same shape as the auto-reinforce sweep) queues a claim job on every rival-owned, non-room `Floor` tile that borders this keeper's own claimed frontier; an Imp standing next to it for the usual claim duration flips its owner (`DungeonGrid.ReclaimTile` — `ClaimTile` still refuses an already-Claimed tile, this is the deliberate exception). The losing keeper's own sweep re-queues it the next tick, so a contested border reads as a slow tug-of-war one tile at a time. `Claim` stays lowest in the job order, so own unclaimed floor and every build job always finish first. Room tiles and Holy Ground are never targets.
...
#### Health / owner ring
A flat "donut" at each creature's feet (`CreatureHealthRing`, built from primitive cubes like `DungeonGrid.BuildGroundStar`) that doubles as the ownership marker and the health bar:
...
On a networked **client** the real species agents don't exist (they run host-side), so `Inspect` falls back to the replicated `CreatureNetView` ghost after the agent rosters come up empty — a shorter readout of species, owner, level, HP, and whether it's knocked out, from `CreatureNetView.All` + its netvars. The **Creatures panel** roster and the **Tasks panel** job list are rebuilt the same way on the client, from data already on the wire — the roster from `CreatureNetView.All` (species, level, owner, HP, coord, and a coarse `CreatureActivity` bucket — Idle / Moving / Working / Hauling / Fighting / Fleeing / Leaving / Down — mirrored host-side from each agent's task enum + combat state; name and precise task aren't replicated). The (cancelable) Dig / Reinforce / Build lists come from a throttled scan of the replicated grid for tiles this keeper queued (`TileState.IsQueuedFor*` / `QueuedByOwnerId`); claim/repair jobs aren't tile-flagged, so only their **counts** ride across, on `KeeperNetState` alongside gold/mana.

`CreatureNetView`'s per-creature netvars are deliberately all small unmanaged types (an enum, ints, floats, two bytes) — `NetworkVariable<FixedString>` has a history of tripping NGO's behaviour-sync buffer here (see the `NetLobby` fix), and a per-creature object is exactly where that would bite hardest.
...
- **`DownedBody`** (`Assets/Scripts/Creatures`) — added to the creature's own GameObject on faint; the agent is **disabled, not destroyed** (so the creature, level, exp, name are all preserved for a clean stand-up — a functional equivalent of the "inert data" model in the design above). 10%-MaxHP faint pool, 60s come-to (routes the revived creature to its Lair to finish healing), 25%/min recovery when delivered to a Lair, permadeath on faint-pool depletion or a Lava/Chasm drop. The placeholder capsule tips on its side while down, and is stood back upright by `MarkJailed` (a jailed prisoner is a live captive, not a body on the floor) as well as by a normal stand-up. **Imps never become a `DownedBody`** — see the Imp-behavior note below.
- **Imp behavior** — Imps flee every hostile except enemy Imps (pathing toward their Throne Room), and fight enemy Imps with "Mine". `OnDisable`/`OnEnable` keep a frozen (grabbed) Imp off the job board. An Imp that loses a fight does **not** drop a `DownedBody` — it's a mana construct, not a rescuable creature, so `Combatant.Faint` destroys it outright and `ImplingAgent.OnDestroy` hands its reserved upkeep mana back to the Throne Room.
- **Rescue Ally / Capture Enemy** — real `JobKind`s in `_jobPriorityOrder` (default at the top, reorderable in the Impling menu like every other kind), but the actual work runs Imp-side: `BuilderJobBoard.TryClaimNearestJob` takes `tryRescue` / `tryCapture` delegates (`ImplingAgent.TryStartRescueJob` / `TryStartCaptureJob`) and invokes each at its kind's spot in the order, since a downed body is a moving entity the coord-keyed board can't track. Capture only starts if the keeper has a Jail with a free pit. New Imp states `MovingToDownedBody` / `CarryingBody`; the body is dragged along, delivered to a Lair tile (`LairManager.TryFindNearestLairTile` → `DownedBody.BeginRecovery`) or a Jail pit. Two Imps race; first to `BeginCarry` wins. A carrier that comes within aggro of a non-Imp hostile drops the body and flees.
...
- **Holy Ground** — white, with a golden 8-pointed star at its center (`DungeonGrid.BuildGroundStar`). Walkable by everyone, same as Floor, but can never be Claimed — `ClaimTile`/`CanBuildRoomOn`/`BordersClaimedTile` are all already gated to `Type == Floor`, so unclaimability falls out of that same check for free rather than needing its own guard. Territory can't grow *through* it either, for the same reason — it's a hole in the claimable map, not just a tile that itself refuses ownership.
- **Unholy Ground** — Holy Ground's dark counterpart: near-black tile, dull-red 8-pointed star (same `BuildGroundStar`, different colour). **Mechanically identical to Holy Ground for now** — walkable by all, unclaimable, territory can't grow through it, all falling out of the same `Type == Floor` gates. A deliberate placeholder; its real identity (evil-creature favour, a ritual surface, …) is still TBD. Wired everywhere Holy Ground is: `TileType`, `IsWalkable`, `EditorCanPlaceRoomOn`, the `[Dev] Terrain` tool, the Level Designer's Map Design menu, save/load, and `ApplyReplicatedTile` — which now also rebuilds the star (and Chasm spikes) on a networked client, where those decorations previously only ever built host-side.
...
No real map generator exists yet, so today these are placed with a dev-only Build-menu tool (`BuildMode.PlaceWater`/`PlaceLava`/`PlaceChasm`/`PlaceHolyGround`/`PlaceUnholyGround`/`PlaceFloor`/`PlaceRock`/`PlaceBedrock`, see `TileInteractionController`/`BottomMenuBar`'s "[Dev] Terrain" buttons) — a placeholder standing in for real procedural placement later. It repaints **any** non-room, non-Bedrock tile freely into any of those types (including back to plain Floor or Rock), free of charge, via `DungeonGrid.DevPaintTerrain` → the same unconditional `Editor*` painters the Level Designer's Map Design menu uses. Bedrock stays its own separate "wall" tool (`SetBedrock`). This applies over the network too — the host runs `DevPaintTerrain` and the tile change replicates like any other.
...
- A Jail's footprint must be pre-dug Claimed Floor this keeper owns, the same `DungeonGrid.CanBuildRoomOn` rule every other room follows (`JailManager.TryPlaceJail`/`CanPlaceFootprint`). It *used* to auto-dig undug Rock on placement, but that quietly destroyed dungeon walls a player had built between rooms, so it now needs the floor dug first like the rest.
...
- **Save/load reconstruction** — `BridgeManager` implements `IRestorableRoomManager` like every other room manager. A saved bridge tile is a `Water`/`Lava` `LevelTileData` carrying a `Bridge_` `RoomId`; both restore paths (`LevelDesignerSession.RestoreTile`, `GameBootstrap.RestoreWorldTiles`) defer it into the footprint map so `RoomReconstruction` dispatches it to `BridgeManager` — one call per tile (`start == end`), gold-free and adjacency-free. The shared `EditorPaintBridgeTile` → `CommitBridgeTile` (claim + per-tile `Bridge_{n}` id + plank mesh + Lava decay timer) backs live placement, restore, and the Level Designer tool. In the per-keeper gameplay load path (`RestoreWorldRoomsPerOwner`) the owner handed to `RestoreRoom` is the *clamped* keeper index, never a raw out-of-range value from the save.
- **Level Designer tool** — the Map Design menu has a **Bridge** tool (`MapDesignTool.Bridge`) that paints a bridge tile onto any Water/Lava tile it's dragged over, owned by the owner selector's current pick — single-tap or drag, no line-lock or adjacency rule (authoring, not a gameplay build). `BridgeManager` in the Level Designer runs with `simulateDecay: false` so a Lava bridge doesn't decay mid-edit. Edit mode reassigns a bridge tile's owner (it selects as a Room — its `Bridge_` RoomId); Remove mode tears it down through `LairManager.EditorRemoveRoomAt` (owner-agnostic) then resets the tile to Rock.

## Saving a game (current implementation)

Two save paths share `LevelData` / `LevelFileIO`:

- **`level1`** — the starting template. Bundled in the build (`Resources/Levels/level1.json`); `GameBootstrap.SaveStartingLevelAsLevel1` only writes it on a truly first-ever install with neither a bundled copy nor a save. "Start Game (offline)" always loads this — a fresh run — and deletes any mid-game save first.
- **`savegame`** — a **mid-game save** (`GameBootstrap.SaveGameSlot`). The Settings menu's **Save game** button calls `GameBootstrap.SaveGame` → `CaptureWorldToLevelData`, which snapshots the live world through a throwaway `LevelDesignerSession` (`CaptureLiveCreatures(buildVisual: false)` + `BuildLevelData`) and then fills in, per keeper from the live `KeeperContext`: gold (`Treasury.TotalGold`), the mana pool (`Throne.MaxMana`), bacon (`Tavern.TotalBacon` → new `LevelPlayerData.StartingBacon`), the palette colour index (`LevelDesignerColors.NearestIndex(ctx.Color)`), and the Throne/Portal structure markers. Every creature's **level + exp** ride along too (`LevelCreatureData.Level`/`Exp`, restored via `Creature.SetProgress` in `RestoreWorldCreatures` — the spawners are `void`, so it grabs the just-spawned agent off its species roster).
